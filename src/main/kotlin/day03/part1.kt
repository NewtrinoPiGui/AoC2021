...
fun part1(input: List<String>): Int {
    val onesCounter = mutableMapOf<Int, Int>()
    input.forEach { reading ->
        reading.mapIndexedNotNull { index, char ->
            if (char == '1') {
                val positionCount = onesCounter.getOrPut(index) { 0 }
                onesCounter[index] = positionCount + 1
            } else {
                onesCounter.computeIfAbsent(index) { 0 }
            }
        }
    }
    val halfCount = input.size / 2
    val gammaRateMap = onesCounter.entries.associate { entry ->
        entry.key to if (entry.value >= halfCount) "1" else "0"
    }
    val epsilonRateMap = onesCounter.entries.associate { entry ->
        entry.key to if (entry.value < halfCount) "1" else "0"
    }
    var gammaRateString = ""
    var epsilonRateString = ""
    (0 until gammaRateMap.size).forEach { index ->
        gammaRateString += gammaRateMap[index] ?: "0"
